...
def test_telemetry_feature_builder_freezes_14d_layout_and_history() -> None:
    ...
    assert np.allclose(features[8:], np.zeros(4, dtype=np.float32))

    builder.observe_action([1.0, -0.25], run_id="run-1")
    next_features = builder.encode(info)
    assert np.allclose(next_features[-2:], np.asarray([1.0, -0.25], dtype=np.float32))

    reset_features = builder.encode({"run_id": "run-2", "speed_kmh": 0.0, "rpm": 0.0, "gear": 0})
    assert np.allclose(reset_features[8:], np.zeros(4, dtype=np.float32))
